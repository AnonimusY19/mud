...
class MicRecorderDelegate extends RecorderDelegate {
  ...
  Future<void> _start(RecordConfig config, {bool isStream = false}) async {
    ...
    // Browser AudioContext rate (often 48000) can differ from RecordConfig
    // (Stream defaults to 44100). Mismatch → low rumble / unintelligible voice.
    final actualSampleRate = context.sampleRate.round();
    final aligned = RecordConfig(
    ...
    final workletNode = await _createWorkletNode(context, aligned);

    // Do not route mic to speakers while recording (avoids feedback/rumble).
    source.connect(workletNode);

    if (!isStream) {
      _encoder?.cleanup();

      if (aligned.encoder == AudioEncoder.wav) {
        _encoder = WavEncoder(
          sampleRate: actualSampleRate,
          numChannels: aligned.numChannels,
        );
      } else if (aligned.encoder == AudioEncoder.pcm16bits) {
        _encoder = PcmEncoder();
      }
    }
